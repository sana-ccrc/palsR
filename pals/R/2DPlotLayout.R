...
SpatialPlotAbsolute = function(model,obs,bench,md,variable,plottype,region){
  ...
  # Fetch colour scheme:
  if((varname=="Qle")|(varname=="VISalbedo")|(varname=="NIRalbedo")){
  ...
  errtext = DrawPlot(region,lon=obs$grid$lon,lat=obs$grid$lat,data=md$modelm,meanval=mean(md$modelm,na.rm=T),sdval=sd(md$modelm,na.rm=T),
                     varname,unitstxt,longvarname,zrange=md$zrange,zcols,title,textloc)
  ##if (region=="Australia"){
  ##  PlotExtremeValue(lon=obs$grid$lon,lat=obs$grid$lat,data=md$modelm,range=md$zrange)
  ##}
  # Second plot: obs 
  ...
  ##if (region=="Australia"){
  ##  PlotExtremeValue(obs$grid$lon,obs$grid$lat,md$obsm,md$zrange)
  ##}
  ...
  ##if (region=="Australia"){
  ##  PlotExtremeValue(obs$grid$lon,obs$grid$lat,(md$modelm-md$obsm),md$diffrange)
  ##}
  ...
  if(bench$exist){
    for(b in 1:bench$howmany){
      ...
      ##if (region=="Australia"){
      ##  PlotExtremeValue(obs$grid$lon,obs$grid$lat,(md$benchm[[b]] - md$obsm),md$diffrange)
      ##}
    }
  }	
...
SpatialPlotRelative = function(model,obs,bench,md,variable,plottype,region){
  ...
  if(bench$exist){
    for(b in 1:bench$howmany){
      ...
      ##if (region=="Australia"){
      ##  PlotExtremeValue(obs$grid$lon,obs$grid$lat,md$benchm[[b]],md$zrange)
      ##}
    }
  }	
