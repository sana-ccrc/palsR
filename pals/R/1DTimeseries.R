...
Timeseries_unc = function(model,obs,bench,variable,plottype,cl,region,outfile){
  ...
  data$bench = PruneBenchmarks(data$obs,data$bench)
  
  # Create data matrix to send to analysis function -> global monthly mean   
  adata = Prepare4DMatrix(data)
  
  # Calculate area weighted mean
  avg_data = AreaWeightedMean(lonlen=obs$grid$lonlen, lat=model$grid$lat, data=adata, mdata=data$model$data[,,1])
  ...
  # Plotting
  if(ncurves==5){
    plotcolours = c("black","grey90","grey90","red","blue")
  }else if(ncurves==6){
    plotcolours = c("black","grey90","grey90","red","blue","green")
  }else if(ncurves==7){
    plotcolours = c("black","grey90","grey90","red","blue","green","yellow")
  }
  plot(xloc,data_series[1,],type="l",ylab=ytext,lwd=3,col=plotcolours[1],ylim=c((ymin),(ymin + (ymax-ymin)*1.2)), 
       xaxt='n',cex.lab=plotcex,cex.axis=plotcex,xlab='',mgp = c(2.5,1,0))
  polygon(c(1:nmonths,rev(1:nmonths)),c(data_series[3,], rev(data_series[2,])),col="grey95",border =NA)
  for(p in 1:ncurves){ # for each additional curve
    lines(data_series[p,],lwd=3,col=plotcolours[p])
  }
  ...
  # Legend
  legend(xmin-(xmax-xmin)*0.03,(ymin + (ymax-ymin)*1.24),legend=legendtext,lty=1,
