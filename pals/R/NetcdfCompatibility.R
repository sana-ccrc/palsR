...
GetVariableDetails = function(request_names){
  ...
  for(v in 1:length(request_names)){
    ...
    if(request_names[v] == 'lat'){
      ...
      var_details[['Name']] = c('SWdown','sfc_sw_down_all_mon','dswrf')
      var_details[['UnitsName']] = c('W/m2','W/m^2','Wm^{-2}','wm2','watt/m^2', 'W m-2')
      ...
      var_details[['Name']] = c('LWdown','sfc_lw_down_all_mon','dlwrf')
      var_details[['UnitsName']] = c('W/m2','W/m^2','Wm^{-2}','wm2','watt/m^2', 'W m-2')
      var_details[['Multiplier']] = c(1,1,1,1,1)
      var_details[['Addition']] = c(0,0,0,0,0)
      var_details[['UnitsText']] = 'W/'~m^{2}
      ...
      var_details[['Range']] = c(-100,100)
    }else if(request_names[v] == 'VISalbedo'){
      var_details[['Name']] = c('VISalbedo')
