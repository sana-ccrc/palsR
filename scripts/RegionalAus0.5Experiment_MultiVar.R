...
if((TypeE$type=="MultiVar") | (TypeE$type=="Single") | (TypeE$type=="Multiple")){
  ...
  for(v in 1:length(EvalProducts)){
    if((EvalProducts[[v]][[1]]$name == "GLEAM") & (EvalProducts[[v]][[1]]$variable == "Qle")){
      ...
      obs = GetMODIS_VISalbedo_QA_Aus(variable=vars[[1]],filelist=EvalProducts[[v]],force_interval='monthly')
      model = GetModelOutput(vars[[1]],ModelOutputFiles)  
      bench = GetBenchmarks(vars[[1]],BenchmarkFiles,nBench)
      # Add those analyses that are equally applicable to any variable to analysis list:
      for(a in 1:length(genAnalysis)){
      ...
      obs = GetMODIS_NIRalbedo_QA_Aus(variable=vars[[1]],filelist=EvalProducts[[v]],force_interval='monthly')
      model = GetModelOutput(vars[[1]],ModelOutputFiles)  
      bench = GetBenchmarks(vars[[1]],BenchmarkFiles,nBench)
      # Add those analyses that are equally applicable to any variable to analysis list:
      for(a in 1:length(genAnalysis)){
      ...
      obs = GetGSCD_Aus(variable=vars[[1]],filelist=EvalProducts[[v]],force_interval='monthly')
      model = GetModelOutput(vars[[1]],ModelOutputFiles)  
      bench = GetBenchmarks(vars[[1]],BenchmarkFiles,nBench)
  ...
  if(EvalProducts[[1]][[1]]$variable == "Qle"){
    ...
    for(v in 1:length(EvalProducts)){  
      if(EvalProducts[[v]][[1]]$name == "MPI_uncertainty"){
        obs[[v]] = GetMPI_uncertainty_Aus(vars[[1]],EvalProducts[[v]],force_interval='monthly')               # with std, 0.5° res
        ...
        if(EvalProducts[[v]][[1]]$mimetype == "application/octet-stream"){
          ...
          obs[[v]] = GetGLEAM_Aus_v2A(variable=vars[[1]],filelist=EvalProducts[[v]],force_interval='monthly')   # no unc., 0.25° res -> 0.5°
        } 
        ## Change resolution of "obs" from 0.25° to 0.5°
        ...
        if(EvalProducts[[v]][[1]]$mimetype == "application/octet-stream"){
          ...
          obs[[v]] = GetGLEAM_Aus_v2B(variable=vars[[1]],filelist=EvalProducts[[v]],force_interval='monthly')   # no unc., 0.25° res -> 0.5°
        } 
        ## Change resolution of "obs" from 0.25° to 0.5°
        obsi=array(NA,dim=c(length(model$grid$lon),length(model$grid$lat),obs[[v]]$timing$tsteps))
        grid.list<- list(x= model$grid$lon, y=model$grid$lat)
        ...
        obs[[v]] = GetMODIS_Aus(variable=vars[[1]],filelist=EvalProducts[[v]],force_interval='monthly')       # no unc., 0.5° res
        
      }else if(EvalProducts[[v]][[1]]$name == "LandFlux"){
        obs[[v]] = GetLandFlux_uncertainty_Aus(variable=vars[[1]],filelist=EvalProducts[[v]],force_interval='monthly')    # with ET_sd, 1° res -> 0.5°
  ...
  for(v in 1:length(EvalProducts)){  
    if(EvalProducts[[v]][[1]]$name == "MPI"){
      obs[[v]] = GetMPI_Aus(vars[[1]],EvalProducts[[v]],force_interval='monthly')               # 0.5° res    
    }else if(EvalProducts[[v]][[1]]$name == "GLEAM_v2A"){
      if(EvalProducts[[v]][[1]]$mimetype == "application/octet-stream"){
        load(EvalProducts[[v]][[1]]$path, envir = e<-new.env())
        obs[[v]]=e$obs
      }else{
        obs[[v]] = GetGLEAM_Aus_v2A(variable=vars[[1]],filelist=EvalProducts[[v]],force_interval='monthly')   # 0.25° res -> 0.5°
      } 
    }else if(EvalProducts[[v]][[1]]$name == "GLEAM_v2B"){
      if(EvalProducts[[v]][[1]]$mimetype == "application/octet-stream"){
        load(EvalProducts[[v]][[1]]$path, envir = e<-new.env())
        ...
        obs[[v]] = GetGLEAM_Aus_v2B(variable=vars[[1]],filelist=EvalProducts[[v]],force_interval='monthly')   # 0.25° res -> 0.5°  
      } 
    }else if(EvalProducts[[v]][[1]]$name == "MODIS"){
      obs[[v]] = GetMODIS_Aus(variable=vars[[1]],filelist=EvalProducts[[v]],force_interval='monthly')       # 0.5° res  
    }else if(EvalProducts[[v]][[1]]$name == "LandFlux"){
      obs[[v]] = GetLandFlux_Aus(variable=vars[[1]],filelist=EvalProducts[[v]],force_interval='monthly')    # 1° res -> 0.5°   
    }
  }
